                     ...
                     const unsigned char *in, unsigned long long inlen)
{
    ...
    if (params->n == 24 && params->func == XMSS_LSH) {
        lsh256_digest(LSH_TYPE_256_256, in, inlen*8, buf);
        memcpy(out, buf, 24);
    }
     if (params->n == 32 && params->func == XMSS_CHAM) {
         tandm_cham(in,inlen, out);
    }
     if (params->n == 32 && params->func == XMSS_LSH) {
         lsh_digest(LSH_TYPE_256_256, in, inlen*8, out);
     }

    else if (params->n == 32 && params->func == XMSS_CHAM) {
        tandm_cham(in, inlen, out);

    }
    else if (params->n == 32 && params->func == XMSS_LEA) {
        tandm_lea(in, inlen, out);
    }
    
    else if (params->n == 64 && params->func == XMSS_LSH) {
        lsh_digest(LSH_TYPE_512_512, in, inlen, out);
    }
